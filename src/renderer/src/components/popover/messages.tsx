...
const Messages = memo(({ aiMessages }: { aiMessages: CoreMessage[] }) => {
      ...
      >
        {aiMessages.map((msg) => {
          if (typeof msg.content === 'string') {
            return <MemoizedMarkdown key={msg.content} id={msg.content} content={msg.content} />
          }

          return msg.content
            .filter((part) => part.type === 'text')
            .map((part, partIndex) => (
              <MemoizedMarkdown key={partIndex} id={partIndex.toString()} content={part.text} />
            ))
        })}
      </Flex>
    </BoxMotion>
...
export const Assistant = () => {
  const { messages, isProcessing } = usePopoverAI()

  const aiMessages = messages.filter((msg) => msg.role === 'assistant')

  const isLoadingFirstResponse = aiMessages.length === 0 && isProcessing
  ...
  return (
    <AnimatePresence>
      ...
      {!isLoadingFirstResponse && messages.length > 0 && <Messages aiMessages={aiMessages} />}
    </AnimatePresence>
  )
}
